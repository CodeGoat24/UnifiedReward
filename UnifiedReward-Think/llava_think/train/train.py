...
from llava_think import conversation as conversation_lib
from llava_think.model import *
from llava_think.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token
                ...
                for dataset in datasets:
                    ...
                    if json_path.endswith(".jsonl"):
                        ...
                        for data in cur_data_dict:
                            ...
                            if 'image' in data:
                                data['image'] = os.path.join(image_folder, data['image'])
                            if 'images' in data:
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            if "image" in sample or "video" in sample or self.data_args.early_mix_text:
                ...
                length_list.append(-cur_len)
        return length_list
    def process_video_with_decord(self, video_file, data_args):
        try:
            import av
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if len(image_file) > 2:
                ...
                image = [(image, images[0].size, "video")]
            else:

                image = [self.process_image(f, "pad") for f in image_file]
                image = [[im[0], im[1], "image"] for im in image]
                
            sources[0]['conversations'][0]['value'] = sources[0]['conversations'][0]['value'].replace("<image>\n", "").replace("\n<image>", "").replace("<image>", "").strip()
            sources[0]['conversations'][0]['value'] = sources[0]['conversations'][0]['value'].replace("<video>\n", "").replace("\n<video>", "").replace("<video>", "").strip()
            sources[0]['conversations'][0]['value'] = "<image>\n" * len(image) + sources[0]['conversations'][0]['value']
            sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
            ...
            try:
                if "shareVideoGPTV" in video_file:
                    ...
                    video, video_time, frame_time, num_frames_to_sample = self.process_video_with_decord(video_file, self.data_args)
        ...
        has_image = ("image" or "images" in self.list_data_dict[i]) or ("video" in self.list_data_dict[i])
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        elif "qwen" or "UnifiedReward" in model_args.model_name_or_path.lower():
